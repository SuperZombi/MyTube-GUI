							...
							<React.Fragment>
								{Object.values(
									subtitles.reduce((acc, item) => {
										...
										const key = item.autogenerated ? `${code}-auto` : code
										if (!acc[key]) {
											acc[key] = {
												code: code,
												name: item.name,
										...
										acc[key].subtitles[item.extension] = item.url
										return acc
									}, {})
								)
		...
		<div className={`stream ${selected && "selected"} ${type == "sub" && "no-hover"}`} onClick={onClick}>
			...
			<div className="container-details">
				...
				<div className="container-details-tags">
					...
					{type == "sub" && (
						Object.entries(info.subtitles).map(([ext, url], index)=>(
							<a key={index} className="subs" href={url} target="_blank">{ext}</a>
						))
					)}
